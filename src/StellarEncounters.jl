export pseudo_mass, solve_xt_new, average_δw2_stars, average_δv2_stars, average_δu2_stars, average_energy_kick_stars, b_min, β_min
...
###################################
_pseudo_mass_I(β::Real, xt::Real, shp::HaloProfile = nfwProfile) = 1.0 - QuadGK.quadgk(lnx-> sqrt(exp(lnx)^2 - β^2)  * ρ_halo(exp(lnx), shp) * exp(lnx)^2 , log(β), log(xt), rtol=1e-10)[1] / μ_halo(xt, shp)
...
"""
function average_δw2_stars(x::Real, xt::Real, β_min::Real, β_max::Real, shp::HaloProfile{<:Real})
    
    function _to_integrate(lnβ::Real) 
        pmI = pseudo_mass_I(exp(lnβ), xt, shp)
...
""" result in (Mpc / s)^2 """
function average_δv2_stars(x::Real, xt::Real, β_min::Real, r_host::Real, subhalo::Halo{<:Real}, host::HostModel{<:Real}, use_tables::Bool = true)
    β_max = (use_tables ? host.maximum_impact_parameter(r_host) : maximum_impact_parameter(r_host, host)) / subhalo.rs
    return average_δu2_stars(r_host, subhalo, host, use_tables) * average_δw2_stars(x, xt, β_min, β_max, subhalo.hp)
end

""" result in (Mpc / s)^2 """
function average_energy_kick_stars(x::Real, xt::Real, β_min::Real, r_host::Real, subhalo::Halo{<:Real}, host::HostModel{<:Real}, θ::Real = π/3, use_tables::Bool = true)
    ...
    return 0.5 * n_stars * average_δv2_stars(x, xt, β_min, r_host, subhalo, host, use_tables)
end

####################################
# Evaluation of β_min

b_min_over_b_max(q::Real, n_stars::Real) = sqrt(1-(1-q)^(1.0/floor(Int, n_stars)))
...
β_min(q::Real, r_host::Real, subhalo::Halo{<:Real}, host::HostModel, θ::Real = π/3, use_tables::Bool = true) = b_min(q, r_host, host, θ, use_tables) / subhalo.rs
